...
main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++)
		if (strcmp(argv[i], "-nostalgia") == 0)
			...
			if (n > 0 && n <= 12)
				...
				fprintf(stderr,
					"9wm: wrong number of virtual screens, must be between 1 and 12\n");
		} else if (argv[i][0] == '-')
			usage();
		else
			break;
	...
	for (;;) {
		...
		case SelectionClear:
			fprintf(stderr,
				"9wm: SelectionClear (this should not happen)\n");
			break;
		case SelectionNotify:
			fprintf(stderr,
				"9wm: SelectionNotify (this should not happen)\n");
			break;
		case SelectionRequest:
			fprintf(stderr,
				"9wm: SelectionRequest (this should not happen)\n");
			break;
		case EnterNotify:
			enter(&ev.xcrossing);
			break;
...
activateprevious()
{
	...
	for (c = clients; c->next; c = c->next) {
		if (c->virtual == virtual && c->state == NormalState)
			tmp = c;
		if (tmp && c->next == current)
			break;
	}

	if (tmp && tmp->state == NormalState && tmp->parent != root) {	/* paranoid */
		active(tmp);
...
activatenext()
{
	...
	for (c = current->next; c != current;
	     c = ((c && c->next) ? c->next : clients))
		if (c && c->state == NormalState && c->virtual == virtual && c->parent != root) {	/* paranoid */
			active(c);
...
{
	fprintf(stderr,
		"usage: w9wm [[-display|-dpy] dpy] [-grey] [-version] [-font fname] [-pass]\n"
...
void
sendcmessage(Window w, Atom a, long x)
{
	XEvent ev;
	int status;
	long mask;
...
/*      XConfigureEvent ce; */
	...
	if (c) {
		...
		if (e->value_mask & CWWidth) {
			c->dx = e->width;
			XResizeWindow(dpy, c->parent, c->dx + 2 * (BORDER - 1),
				      c->dy + 2 * (BORDER - 1));
		}
...
mapreq(XMapRequestEvent *e)
{
	...
	case WithdrawnState:
		...
		XReparentWindow(dpy, c->window, c->parent, BORDER - 1,
				BORDER - 1);
...
clientmesg(XClientMessageEvent *e)
{
	...
	if (e->message_type == wm_change_state) {
		...
		if (e->format == 32 && e->data.l[0] == IconicState && c != 0) {
			...
			fprintf(stderr,
				"9wm: WM_CHANGE_STATE: format %d data %d w 0p%p\n",
				e->format, (int)e->data.l[0],
				(void *)e->window);
		return;
	}
