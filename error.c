...
handler(Display *d, XErrorEvent *e)
{
	...
	if (initting && (e->request_code == X_ChangeWindowAttributes)
		...
		fprintf(stderr,
			"9wm: it looks like there's already a window manager running;  9wm not started\n");
		exit(1);
	}
	...
	XGetErrorDatabaseText(d, "XRequest", number, "<unknown>", req,
			      sizeof(req));

	fprintf(stderr, "9wm: %s(0x%x): %s\n", req, (unsigned int)e->resourceid,
		msg);

	if (initting) {
		fprintf(stderr,
			"9wm: failure during initialisation; aborting\n");
		exit(1);
	}
	return 0;
}
...
void
showhints(Client *c, XSizeHints *size)
{
#ifdef  DEBUG
	fprintf(stderr, "\nNew window: %s(%s) ", c->label ? c->label : "???",
		c->class ? c->class : "???");
	...
	if (size->flags & PMinSize)
		fprintf(stderr, "PMinSize(%d,%d) ", size->min_width,
			size->min_height);
	if (size->flags & PMaxSize)
		fprintf(stderr, "PMaxSize(%d,%d) ", size->max_width,
			size->max_height);
