...
int
grab(Window w, int mask, Cursor curs, int t)
{
	int status;
...
void
draw_text(int *been_here, int wide, int high, int n, int cur, Menu *m)
{
	int i;
	int tx, ty;
	...
	*been_here = 1;
	...
	for (i = 0; i < n; i++) {
		tx = (wide -
		      XTextWidth(font, m->item[i], strlen(m->item[i]))) / 2;
		ty = i * high + font->ascent + 1;
		XDrawString(dpy, menuwin, gc, tx, ty, m->item[i],
			    strlen(m->item[i]));
	...
	for (;;) {
		...
		case MotionNotify:
			...
			if (cur == old)
				break;
			if (old >= 0 && old < n)
				XFillRectangle(dpy, menuwin, gc, 0, old * high,
					       wide, high);
			if (cur >= 0 && cur < n)
				XFillRectangle(dpy, menuwin, gc, 0, cur * high,
					       wide, high);
			break;
		case Expose:
			draw_text(&drawn, wide, high, n, cur, m);
...
Client *
selectwin(int release, int *shift)
{
	XEvent ev;
	XButtonEvent *e;
	int status;
...
sweepdrag(Client *c, XButtonEvent *e0, void (*recalc)())
{
	...
	for (;;) {
		...
		case ButtonRelease:
			...
			c->dx -= 2 * BORDER;
			c->dy -= 2 * BORDER;
			if (c->dx < 4 || c->dy < 4 || c->dx < c->min_dx
			    || c->dy < c->min_dy)
				goto bad;
			return 1;
		}
